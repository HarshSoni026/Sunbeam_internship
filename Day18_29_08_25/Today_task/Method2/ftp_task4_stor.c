// ftp_stor_simple.c
// Minimal beginner-level STOR demo (single connection)
...
int main() {
    ...
    while (1) {
        ...
        printf("Client> %s", buffer); // Show what client typed

        if (strncmp(buffer, "USER", 4) == 0) {
            write(client_fd, "230 Login OK\r\n", 14);
        } 
        // Task 4 - APPEND command
        else if (strncmp(buffer, "APPE", 4) == 0) {
            char filename[100];
            if (sscanf(buffer, "APPE %s", filename) != 1) {
                write(client_fd, "501 Syntax error\r\n", 19);
                continue;
            }
            int fp = open(filename, O_WRONLY | O_APPEND | O_CREAT, 0644);
            if (fp == -1) {
